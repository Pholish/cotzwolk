...
$(document).ready(function(){
	...
	{
		$('body').removeClass('tablet_horizontal');
		$('body').addClass('tablet_vertical');
		...
		$("body").on("click", ".head-menu-list__link,.menu-list__link,.socials__link,.__click-enabled", function (event) {
		...
		if ($(this).attr('href').indexOf(".php") > -1)
		{
			...
			$("body").on("click", ".socials__link,.__click-enabled", function (event) {
...
*/
	...
	var contactsMap,
		marker;
	var image = rootPath+'images/map-marker.svg',
		markersData = [
			{
				lat: 55.804960,
				lng: 37.590046,
				title: 'Catzwolf Digital',
				address: 'г. Москва,ул. Новодмитровская, д. 2, стр. 1'
 
			}
		], contactsMap, infoWindow;

	function initContactsMap() {
		var centerLatLng = new google.maps.LatLng(55.804960, 37.590046);
		var mapOptions = {
			center: centerLatLng,
			zoom: 14,
			scrollwheel: false,
			...
			styles: [
 {
    ...
    },
    {
		...
		google.maps.event.addListener(contactsMap, 'click', function() {
			infoWindow.close();
		});
		...
		for (var i = 0; i < markersData.length; i++){
			...
			addMarker(latLng, title, address, i);
		}
	
	}


	function addMarker(latLng, title, address, i) {
		...
		var contentString = '<div class="infowindow__title">' + title + '</div>' + '<div class="infowindow__address">' + address + '</div>';
		...
		infoWindow.close(contactsMap, marker);
	}
